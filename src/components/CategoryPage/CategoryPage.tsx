...
interface CategoryPageState {
    isUserLoggedIn: boolean;  // setLoginState
    category?: CategoryType;  // setCategoryData
    subcategories?: CategoryType[];  // setSubcategories
    articles?: ArticleType[];  // setArticles
    message: string;   // setMessage
    filters: {
        keywords: string;
        priceMinimum: number;
        priceMaximum: number;
...
// data that we are getting from backend server api for subcategory
interface ApiSubcategoryDto {
    categoryId: number,
    name: string,
}

// data that we are getting from backend server api for article search
...
export default class CategoryPage extends React.Component<CategoryPageProperties> {
    ...
    private featuresFilterChanged(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    private removeFeatureFilterValue(featureId: number, value: string) {
        const newSelectedFeatures = this.state.filters.selectedFeatures.filter(
            record => {
    ...
    private setSelectedFeatures(newSelectedFeatures: { featureId: number; value: string; }[]) {
        this.setState(Object.assign(this.state, {
        ...
        return (
            <Form.Group>
                ...
                {feature.values.map(value =>
                    this.printFeatureFilterCheckBox(feature, value), this)}
            </Form.Group>
        );
    }
    ...
    private printFeatureFilterCheckBox(feature: any, value: string) {
        return (
            <Form.Check type="checkbox" label={value}
                ...
                onChange={(e: any) => this.featuresFilterChanged(e as any)} />
        );
    }
        ...
        api('api/category/' + this.props.match.params.cId, "get", {})
            .then((res: ApiResponse) => {
                ...
                const subcategories: CategoryType[] =
                    res.data.categories.map((category: ApiSubcategoryDto) => {
                        return {
